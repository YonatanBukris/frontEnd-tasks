        ...
        <Dialog>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleCreateTask} className="space-y-4 text-black">
              ...
              <Button type="submit" className="p-2bg-slate-500  rounded-md">Create Task</Button>
            </form>
          </DialogContent>
        </Dialog>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {pinnedTasks.map(task => (
              <Link key={task._id} to={`/task/${task._id}`}>
                <Card className={`border border-gray-300 shadow-md cursor-pointer relative ${areAllTodosComplete(task) ? 'bg-green-800' : ''}`}
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {nonPinnedTasks.map(task => (
              <Link key={task._id} to={`/task/${task._id}`}>
                <Card
                  className={`border border-gray-300 shadow-md cursor-pointer relative ${areAllTodosComplete(task) ? 'bg-green-800' : ''}`}
                  onMouseEnter={() => setHoveredTask(task._id)}
                  onMouseLeave={() => setHoveredTask(null)}
                >
                  <CardHeader>
